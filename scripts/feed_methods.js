...
const stakePancake = false, stakeBossCard=false,revealReward=true, getUserFeeds=false; //upgradeContract=true
const runUpdateMethods = false;
...
async function main(){
    ...
    if(runUpdateMethods){
        const responseAddress = await DeployedContract.getApplyContracts()
        console.log("response",responseAddress)
        const  userFeeds = await DeployedContract.getUserFeeds(process.env.OWNER)
    ...
    if(revealReward){
      ...
      /*  var count = 0;
        ...
        console.log("bossTokens--- before",bossTokens);

        let input1 = [1,1,1,1,1],input2 = [0,1],input3 = [1,0];
       ...
       // console.log("reveal done",response);
        ...
        const bossAfterTokens = await DeployedBossContract.getWalletToken();
        console.log("bossAfterTokens--- after",bossAfterTokens);
       /* var ingredientTokens = await DeployedIngredientContract.getWalletToken()
