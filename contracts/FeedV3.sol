...
interface INewPancakeERC1155 {
    function safeTransferFrom(address from, address to, uint id, uint amount, bytes memory data) external;
}

contract FeedV2 is Initializable, OwnableUpgradeable,ERC1155HolderUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    address private newPancakeERC1155;

    function initialize(address _pancakeERC1155, address _ingredientsERC1155, address _bossCardERC1155, address _commonConst, address _signatureChecker, address _newPancakeERC1155) external initializer {
        __Ownable_init();
        ...
        newPancakeERC1155 = _newPancakeERC1155;
    }

    function updateContractAddress (address _pancakeERC1155, address _ingredientsERC1155, address _bossCardERC1155, address _commonConst, address _signatureChecker, address _newPancakeERC1155) external onlyOwner{
        pancakeERC1155 = _pancakeERC1155;
        ingredientsERC1155 = _ingredientsERC1155;
        bossCardERC1155 = _bossCardERC1155;
        commonConst = ICommonConst(_commonConst);
    ...
    function existToken(uint[] memory tokenIds) private pure returns (bool) {
        bool exist = false;
    ...
    function stake(uint[] memory tokenIds, uint[] memory amounts, uint calories, bytes memory signature) external nonReentrant whenNotPaused {
        ...
        if(existToken(tokenIds)){
            for(uint i = 0; i < tokenIds.length; i++){
                if(tokenIds[i] >= 20){ 
                    INewPancakeERC1155(newPancakeERC1155).safeTransferFrom(msg.sender, address(this), tokenIds[i], amounts[i],'');
                }
                else{
    ...
    function getApplyContracts() external view returns(address,address,address) {
        return(pancakeERC1155,ingredientsERC1155,bossCardERC1155);
    }
