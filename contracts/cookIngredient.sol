...
contract CookIngredients is  ERC1155, ERC1155Burnable, ReentrancyGuard, Ownable, Pausable{
    ...
    uint legendaryIngredientId = 25;
    uint8 basiPancakeRecipeId = 1;
    ...
    function stake(uint256[] memory tokenIds, uint8 cliamTokenId, bytes32 _signedhashMessage, bytes memory _signature) external nonReentrant whenNotPaused{
    ...
    function bossCardWithdraw(uint _tokenId, bytes32 _signedhashMessage, bytes memory _signature) external{
        require(
            !anyClaimInProgress(),
            "Claim in progress"
        );
